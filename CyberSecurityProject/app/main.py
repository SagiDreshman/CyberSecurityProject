...
def list_users(db: Session = Depends(get_db), token_user=Depends(require_roles("admin"))):
    # token_user הוא dict: {username, role}
    db_user = db.query(models.User).filter(models.User.username == token_user["username"]).first()

    add_log(db, user_id=db_user.id if db_user else None, action="ListUsers", status="SUCCESS")

    users = db.query(models.User).order_by(models.User.id.asc()).all()

    # מחזירים רק מה שצריך (כדי שיהיה נקי ובטוח)
    return [{"id": u.id, "username": u.username, "role": u.role} for u in users]


@app.put("/admin/orders/{order_id}/status")
def admin_update_order_status(order_id: int, req: models.UpdateOrderStatusRequest, db: Session = Depends(get_db), user=Depends(require_roles("admin"))):
    if req.status not in ALLOWED_ORDER_STATUS:
