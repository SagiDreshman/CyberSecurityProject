...
from app.cert_service import issue_user_certificate
...
secure_router = APIRouter(
    dependencies=[Depends(require_client_certificate)]
)
def get_db():
    db = SessionLocal()
    try:
...
def register(req: RegisterRequest, db: Session = Depends(get_db)):
    ...
    # ✅ מנפיק תעודה (אם אין כבר) ומחזיר PEM
...
def login(
    ...
    cert_cn: str = Depends(require_client_certificate)  # ✅ חובה תעודה
):
    # ✅ התעודה חייבת להתאים למשתמש שמנסה להתחבר
...
def analyze_ai(
    ...
    for l in logs:
        ...
        r = detector.score_one(text, threshold=0.65)
        results.append({
            "log_id": int(l.id),
            "action": str(l.action),
            "status": str(l.status),
